...
Page({
  ...
  chooseDatingMethod: function(event) {
    ...
    _this.setData({
      datingMethod: method,
      showHideBG: method == 8 ? true : false
    })
  },
  ...
  // 选择出发时间
  selectDepartureTime: function(event) {
    const _this = this;
    wx.showActionSheet({
      itemList: _this.data.departureTimeMenu,
      itemColor: '#000',
      success(res) {
        if (res.tapIndex == 2) {
          _this.setData({
  ...
  onPickerDatingTime: function(event) {
    ...
    console.log(this.data.datingTimeText)
  },
  ...
  finish: function(event) {
    ...
    if (state == 1) {
      ...
      if (!_this.data.extInt7) {
        wx.showToast({
          icon: 'none',
          title: '请选择约会时间',
        })
        return;
      } else if (_this.data.extInt7 == 3) {
