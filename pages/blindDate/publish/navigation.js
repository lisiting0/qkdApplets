...
Page({
  ...
  onLoad: function(options) {
    wx.setNavigationBarTitle({
      title: "地址导航" //页面标题为路由参数
    })
    let that = this; 
    let pubData = app.globalData.publishBlindDate; 
    let navigationStorage = pubData.navigation;
    if (navigationStorage) {
      //将br替换成empty
      let newStorage = navigationStorage.replace("<br/>", "empty");
      // 正则提取数据 
  ...
  editorConfirm: function() {
    ...
    app.globalData.publishBlindDate.navigation = html;
    wx.navigateBack({
      delta: 1
    })
  }
